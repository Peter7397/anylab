...
Return ONLY valid JSON array, no other text:"""
        ...
        if not self.model_name:
            logger.error("Ollama model is not set in GraphEntityExtractor!")
            raise ValueError("Ollama model is not configured. Please set a model in System Settings.")
        
        try:
            api_url = f"{self.ollama_url}/api/chat"
            logger.debug(f"Calling Ollama API for entity extraction: {api_url} with model: {self.model_name}")
            ...
            if not response_text:
                logger.error(f"Empty response from Ollama for entity extraction. Response: {response_data}")
            ...
            if e.response.status_code == 404:
                logger.error(f"Ollama API endpoint not found (404). URL: {api_url}, Model: {self.model_name}. Check if Ollama is running.")
            else:
                logger.warning(f"HTTP error from Ollama during entity extraction: {e.response.status_code} - {e.response.text}")
        except requests.exceptions.ConnectionError as e:
            logger.warning(f"Cannot connect to Ollama at {self.ollama_url} for entity extraction. Make sure Ollama is running.")
